...
public class HexDumpOwn {
    ...
    static int addressLength = 4;
    //static int addressLength = 8;
    ...
    public static String prettyPrint(byte[] input) {
        ...
        if (printHeaderBool == true) {
            if (printDecimalAddressBool == true) {
                ...
                laengeVorspannInt = laengeVorspannInt + (addressLength + 1);
            }
        ...
        for (int i = 0; i < filesizeLong; i++) {
            ...
            if (printDecimalAddressBool == true) {
                outputLineString = outputLineString + formatMitNullenLinks(String.valueOf(adresseInt), addressLength) + ":";
            }
            // ausgabe der adresse als hexwert
            if (printHexAddressBool == true) {
                outputLineString = outputLineString + formatMitNullenLinks(Integer.toHexString(adresseInt), addressLength) + ":";
            }
            asciiZeileString = "";
